...
- Миграция `0005_create_admin_tables.sql` (orders, order_items, discount_codes, store_settings) - пусната през Supabase SQL Editor (Dashboard → SQL Editor → paste съдържанието на файла → Run); така се пускат всички следващи миграции, за да не се налага DB паролата
- Миграция `0011_create_users.sql` - таблица `public.users` (профилни данни: email, full_name, phone, avatar_url), 1 ред на auth потребител. Trigger `on_auth_user_created` я попълва автоматично при регистрация (от Google идват име + снимка), плюс backfill за съществуващите. **Профилът се чете/пише оттук** (`src/lib/account/profile.ts`, `src/lib/account/actions.ts`, `src/lib/profile-context.tsx`), не от `auth.users.raw_user_meta_data`; в Supabase Auth остават само credentials/сесия (смяна на парола). Снимките са в Storage bucket `avatars` (качване server-side през service role - `uploadAvatar`)
