 ...
 * before anything is written, same pattern as admin server actions.
 *
 * Never throws: a thrown error in a Server Action surfaces on the client
 * as an opaque "Minified React error #441" in production (the real
 * message is redacted), so every failure is returned as { error } and
